...
func TestMigrationPreDumpSettings(t *testing.T) {
	tests := []struct {
		...
		{name: "unset", config: map[string]string{}},
		{name: "disabled", config: map[string]string{"migration.incremental.memory": "false"}},
		{
			name:           "unsupported",
			config:         map[string]string{"migration.incremental.memory": "true"},
			supportErr:     errors.New("unsupported"),
			expectedChecks: 1,
		},
		{
		...
		{
			name: "iterations capped",
			config: map[string]string{
				"migration.incremental.memory":            "true",
				"migration.incremental.memory.iterations": "1000",
			},
			expectedEnabled:    true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			checks := 0
			enabled, iterations := migrationPreDumpSettings(tt.config, func() error {
				checks++
				return tt.supportErr
			})

			if enabled != tt.expectedEnabled || iterations != tt.expectedIterations {
				t.Fatalf("Pre-dump settings = (%t, %d), want (%t, %d)", enabled, iterations, tt.expectedEnabled, tt.expectedIterations)
			}

			if checks != tt.expectedChecks {
				t.Fatalf("CRIU support checks = %d, want %d", checks, tt.expectedChecks)
			}
		})
	}
}
